...
this.setData({
 ...
 getinfo(){
  ...
  //console.log(wx.getStorageSync('video'),'video')
  //console.log(Math.round(wx.getStorageSync('duration')*60),'duration')
    this.setData({
      video:wx.getStorageSync('video'),
      timeout:Math.round(wx.getStorageSync('duration')*60),
      timeouts:Math.round(wx.getStorageSync('duration')*60)+0.29
    })
   // console.log(this.data.timeout,this.data.timeouts)
  },
  /**
   * 生命周期函数--监听页面初次渲染完成
   */
